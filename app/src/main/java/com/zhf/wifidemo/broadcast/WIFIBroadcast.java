...
public class WIFIBroadcast extends BroadcastReceiver{
	...
	public void onReceive(Context context, Intent intent) {
		...
		if(intent.getAction().endsWith(WifiManager.SCAN_RESULTS_AVAILABLE_ACTION)) {
			Log.i(TAG,"WTScanResults---->扫描到了可用网络---->android.net.wifi.SCAN_RESULTS");
			//遍历通知各个监听接口
		...
		//wifi打开或关闭状态变化   "android.net.wifi.WIFI_STATE_CHANGED"
			...
			Log.i(TAG,"WTScanResults----->wifi状态变化--->android.net.wifi.WIFI_STATE_CHANGED");
		...
		//连接上一个SSID后发出的广播，(注：与android.net.wifi.WIFI_STATE_CHANGED的区别)  
			...
			Log.i(TAG,"WTScanResults----->网络状态变化---->android.net.wifi.STATE_CHANGE");
